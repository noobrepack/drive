...
if($method === 'POST') {
    ...
    try {
        success(encrypt($requestData['folder'], 'u9v5d/DUe0mpLhuqcLaN8WPRnoppwsNjmL2iWS0faQY='));
    }
    catch(Exception $e) {
        fail($e.getMessage());
    }
...
document.querySelector("#encrypt").addEventListener("click", async function(event) {
  ...
  try {
    document.querySelector("#encrypted_link").value = "ZGVjcnlwdGlvbi5ub29iZ2FtaW5nLndvcmtlcnMuZGV2." + (await encryptFolder());
  }
  catch(e) {
    alert(e.message);
  }
